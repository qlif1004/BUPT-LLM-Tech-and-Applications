        ...
        messages = [
            {
                ...
                "content": (
                    ...
                    "schedule 字段必须严格遵循以下格式之一："
                    "1) \"manual\" 表示手动触发；"
                    "2) \"weekly [星期] [时间]\" 如 \"weekly mon 08:00\"、\"weekly 周一 08:00\"；"
                    "3) \"daily [时间]\" 如 \"daily 09:00\"、\"daily 每天 09:00\"；"
                    "4) \"monthly [日期] [时间]\" 如 \"monthly 1日 08:00\"；"
                    "5) \"cron: [5段表达式]\" 如 \"cron: 0 8 * * mon\"。"
                    "如果用户未提及调度频率，默认输出 \"manual\"。"
                    "只输出 JSON，不要输出解释。"
                ),
            },
